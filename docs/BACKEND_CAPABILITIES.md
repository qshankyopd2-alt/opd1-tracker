...
| `GET /api/live?seed&state` | full board (above) + `session`, `recap` (after a match), `notice`, `encounter`, `saved`, and `savedNote` per player, plus optional `inferredGroups` derived from already-fetched historical match details. A group means the listed current players shared a historical team in one or more match IDs; `partyMatches` counts historical records with the same non-empty party ID. It is evidence, not proof of a current party, and is refreshed when profile history (including matches 6–8) is opened. Returns an OFFLINE board (`source:"local"`, empty players, client notice) when the client isn't running or ready (demo fallback removed) |
