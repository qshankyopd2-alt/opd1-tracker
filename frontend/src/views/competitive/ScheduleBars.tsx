    ...
    <div className="space-y-1.5" data-testid={testId}>
      {rows.map((r) => (
        <div key={r.name} className="flex items-center gap-2 text-[12px]">
          ...
          <span className="w-20 text-right num text-zinc-400 shrink-0">
...
export function ScheduleBars({ weekdays, dayparts }: { weekdays: ScheduleRow[]; dayparts: ScheduleRow[] }) {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-4">
      <div>
        <h4 className="text-[12px] text-zinc-400 mb-2">By weekday</h4>
        <Bars rows={weekdays} testId="schedule-weekdays" />
      </div>
      <div>
        <h4 className="text-[12px] text-zinc-400 mb-2">By time of day</h4>
