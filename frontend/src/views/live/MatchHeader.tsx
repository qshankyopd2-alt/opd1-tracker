      ...
      <div className="relative flex items-center gap-6 px-4 py-3 min-h-[86px]">
        <div className="min-w-0">
          <div className="flex items-center gap-2.5">
            <div className="rounded-sm px-2 py-0.5 text-[10px] font-black uppercase tracking-widest text-ink shadow-sm" style={{ backgroundColor: STATE_COLOR[board.state] ?? "#71717A" }} data-testid="match-state-badge">
              {board.stateLabel}
            </div>
            {board.state !== "MENUS" && (
          ...
          <h1 className="mt-1 font-display font-black italic text-[36px] leading-none uppercase tracking-tight text-white drop-shadow-md">
        ...
        <div className="ml-auto flex items-center gap-10">
          {score && (
            <div className="text-center" data-testid="match-score">
              <div className="font-display font-black text-[42px] leading-none num drop-shadow-md flex items-center justify-center gap-3">
          ...
          {lock && (
            <div className="text-center bg-panel/40 px-5 py-2 rounded-md border border-edge/50 backdrop-blur-sm" data-testid="lock-progress">
              <div className="font-display font-black text-[36px] leading-none num text-brand drop-shadow-sm">
          ...
          {board.winProb !== null && board.state === "INGAME" && (
            <div className="w-48 bg-panel/30 p-2.5 rounded-md border border-edge/50 backdrop-blur-sm" data-testid="win-probability">
              ...
              <div className="h-2 bg-zinc-800/80 rounded-full overflow-hidden border border-zinc-900 shadow-inner">
                <div
                  className="h-full rounded-full transition-[width] duration-700 ease-out"
                  style={{
                    width: `${board.winProb}%`,
                    backgroundColor: board.winProb >= 55 ? "#10B981" : board.winProb <= 45 ? "#EF4444" : "#F59E0B",
                    boxShadow: `0 0 10px ${board.winProb >= 55 ? "#10B981" : board.winProb <= 45 ? "#EF4444" : "#F59E0B"}80`,
                  }}
                />
              </div>
            </div>
