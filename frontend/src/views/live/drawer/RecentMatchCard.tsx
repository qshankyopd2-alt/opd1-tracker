...
export function RecentMatchCard({ match, onOpen }: { match: CareerMatch; onOpen: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      className="group relative grid h-[68px] w-full grid-cols-[3px_28px_minmax(0,1fr)_auto_auto_auto_minmax(96px,116px)] items-center gap-2 overflow-hidden border-b border-edge/70 bg-card/35 px-3 py-2 text-left transition-colors hover:bg-zinc-800/70"
    ...
    >
      ...
      <span className="relative z-10 whitespace-nowrap font-mono text-[12px] text-zinc-200 num">{match.kills}/{match.deaths}/{match.assists}</span>
      <span className="relative z-10 font-mono text-[12px] text-zinc-300 num">{match.acs}</span>
      <span className={`relative z-10 whitespace-nowrap font-mono text-[12px] font-semibold num ${match.rrDelta !== null && match.rrDelta !== undefined && match.rrDelta >= 0 ? "text-victory" : "text-defeat"}`}>
