      ...
      <div className="mt-2 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={savedBusy}
            onClick={onSave}
            className="inline-flex items-center gap-1.5 rounded-sm bg-amber-300 px-2.5 py-1.5 text-[11px] font-bold uppercase tracking-wider text-ink disabled:opacity-50"
          >
          ...
          {saved && (
            <button
              ...
              disabled={savedBusy}
              onClick={onRemove}
              className="inline-flex items-center gap-1.5 rounded-sm border border-edge px-2.5 py-1.5 text-[11px] font-semibold uppercase tracking-wider text-zinc-400 hover:text-defeat disabled:opacity-50"
            >
              <Trash2 size={11} /> Remove
            </button>
          )}
        </div>
        <span className="text-[9px] text-zinc-600 num">{note.length}/500</span>
      </div>
