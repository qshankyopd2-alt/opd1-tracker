        ...
        {teammates.map((teammate) => (
          <li key={teammate.puuid} className="min-w-0 border-b border-r border-edge/60 px-2.5 py-2.5 even:border-r-0 max-[560px]:border-r-0">
            <div className="flex min-w-0 items-baseline gap-2">
              <span className="min-w-0 flex-1"><Truncate text={teammateName(teammate)} maxWidth={180} className="text-[12px] font-semibold text-zinc-100" /></span>
              <span className="shrink-0 text-[10px] text-zinc-400 tabular-nums">{teammate.sharedMatches} {teammate.sharedMatches === 1 ? "match" : "matches"}</span>
            </div>
