...
function MatchRow({
  ...
  return (
    <button
      ...
      className="group relative min-h-[92px] w-full overflow-hidden rounded-md border border-white/10 bg-card text-left transition-colors hover:border-white/20 hover:bg-card-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand"
    >
      {splash && (
    ...
    <div className="p-5 space-y-4" data-testid="history-view">
      <PageHeader title="Match history">
        <div className="flex border border-edge rounded-sm" data-testid="history-filter">
          {(["all", "wins", "losses", "bookmarked"] as Filter[]).map((f) => (
            <button
              ...
              onClick={() => setFilter(f)}
              className={`px-3 py-1.5 text-[12px] font-semibold capitalize transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand ${
        ...
        <button
          ...
          onClick={refresh}
          className="ml-auto inline-flex items-center gap-1.5 rounded-sm border border-edge px-3 py-1.5 text-[12px] font-semibold text-zinc-300 transition-colors hover:bg-zinc-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand"
        >
