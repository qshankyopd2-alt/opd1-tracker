...
def settings_post():
    ...
    with _SETTINGS_LOCK:
        ...
        try:
            ...
            return jsonify({"ok": False, "message": "An error occurred while saving settings.",
                            "settings": merged}), 200
        ...
        with _BUILD_LOCK:
            ...
            try:
                ...
                if client.source_pref == "local":
                    app.logger.exception("Failed to build live scoreboard")
                    return {"state": "OFFLINE", "stateLabel": "Offline", "source": "local",
                            "error": "Failed to connect to local client.", "players": [], "teams": {}, "parties": [],
...
def state():
    ...
    if _live_enabled():
        try:
            ...
            app.logger.exception("game state check failed")
            return jsonify({"state": "OFFLINE", "stateLabel": "Offline",
                            "source": "local", "error": "Failed to connect to local client."})
...
def debug_reveal():
    ...
    try:
        ...
        return jsonify({"error": "An internal error occurred during debug reveal."}), 500
